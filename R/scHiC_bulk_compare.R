...
#' @param D.interval A numeric vector defining the distance intervals to consider in the analysis, or a character string 'full' indicating the inclusion of all genomic distances in the analysis. The distance should be scaled by dividing by the data resolution, D = (start2 - start1)/resolution (e.g., D = (16,000,000 - 17,000,000)/1,000,000 -> D = 1). [??? What is the default? 'full'?]
#' @param fprControl.logfc A numeric value controlling the false positive rate by setting the threshold for the log fold change. Detected differences should have a value larger than this threshold. [??? An example of how this is used with the log fold change would help to clarify]
#'                       Default is 0.8.
#' @param alpha A numeric value for the significance level of outlier detection. Default is 0.05. [??? how does this relate to `pools_impute()` outlier detection?]
...
#' @param numChanges An integer indicating the number of changes to add to the Hi-C matrix. This value is used to identify the optimal 'A.min' quantile cutoff in the `filter_params()` and `hic_compare()` functions for detecting significant outliers. The number of changes should be proportional to the resolution of the data. High-resolution data should use more changes (e.g., 1MB resolution - 300 changes, 100KB resolution - 1000 changes, etc.). Default is 300. [??? If I were an end user, how would I know how to calculate this? Consider writing an algorithm for calculating this on the fly by default]
#' @param FC A numeric value representing the fold change threshold added to the Hi-C matrix. This value is used to identify the optimal 'A.min' quantile cutoff in the `filter_params()` and `hic_compare()` functions for detecting significant outliers. Default is 3. [??? How does changing this affect the threshold? e.g. If this value is higher, it produces a higher 'A.miin' quantile cutoff? When should I change this?]
...
#'                 Default is FALSE. This option only works on Unix-based operating systems and is useful when analyzing a list of Hi-C tables. [??? Should this be TRUE by default?]
...
#'
#' @details 
#' The `BulkHiC_compare()` function [??? was this renamed to `scHiC_bulk_compare()`?] performs differential chromatin interaction comparisons between single-cell Hi-C data from two cell type groups or conditions. The workflow includes clustering normalized log fold changes between interaction frequencies of 2 cells type at each genomic distance into "difference" and "non-difference" groups. The non-difference group is assumed to follow a normal distribution centered around 0 and is clustered by a Gaussian Mixture Model. The difference cluster consists of points belonging to other distributions. In cases where the size of the differences is not large enough to form distinct distributions, these differences are likely outliers of the normal distribution, which are identified by the `hic_compare()` function.
#' 
#' @references
#' 
...
# [??? where is the parallel parameter?]
