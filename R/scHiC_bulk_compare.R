...
#'                       Default is 0.8.
#' @param alpha A numeric value for the significance level of outlier detection in the `hic_compare()` function from HiCcompare. The default is 0.05.
...
#' @param SD A numeric value specifying the standard deviation threshold for fuzzing, used to produce a simulated Hi-C matrix. This value is used to modify the process of finding the optimal 'A.min' quantile cutoff for detecting significant outliers. Users can select the value based on their assumption of the scHi-C data. The default is 2.
#' @param numChanges An integer or NULL, indicating the number of changes to add to the simulated Hi-C matrix. This value is used to modify the process of finding the optimal 'A.min' quantile cutoff for detecting significant outliers. Based on the user's assumption about the possible number of differences, they can set the number of changes that should be proportional
#'  to the resolution of the data. High-resolution data should be assumed to have more changes. If `numChanges` = NULL, the function is set to the number of changes (or simulated difference) scaled by a factor of 30 (e.g., 1MB resolution - 30 changes, 500KB resolution - 60 changes, etc.) The default is NULL.
#' @param FC A numeric value representing the fold change threshold added to the simulated Hi-C matrix. This value is used to identify the optimal 'A.min' quantile cutoff for detecting significant outliers. Users can select the FC value based on their assumption of difference fold change in their data. The default is 3.
