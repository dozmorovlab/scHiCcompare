...
#' @param color_low A character string specifying the color for the lowest
...
#' @param color_high A character string specifying the color for the highest
...
#'   by running `colors()` in R.
#' @param main A character string for the main title of the plot. The default
#'   is NULL.
#' @param figure_name A character string for additional figure labeling. The
#'   default is NULL.
#'
...
#'
#' @export
plot_HiCmatrix_heatmap <- function(scHiC.sparse,
                                   zlim = NULL,
                                   color_low = "white",
                                   color_high = "red",
                                   main = NULL,
  ...
  color_scale <- grDevices::colorRampPalette(c(color_low, color_high))
  ...
  lattice::levelplot(
    ...
    col.regions = colors,
    at = seq(log(zlim[1] + 1e-6), log(zlim[2] + 1e-6),
