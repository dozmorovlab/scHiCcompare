...
Each '.txt' data of a single-cell HiC should be formatted as modified sparse upper triangular matrices in R, which consist of five columns (chr1, start1, chr,2 start2, IF). Since the full matrix of chromatin interactions is symmetric, only the upper triangular portion, including the diagonal and excluding any 0, is stored as sparse matrix format. The required sparse matrix format of each single cell Hi-C is: 
...
Since the data download from `r BiocStyle::Biocpkg("Bandnorm")` has achieved the required input format (5 columns of [chr1, start1, chr2, start2, IF]) for `ScHiCcompare()` function, we don't need extra step for data modification. If after importing your data into R, its format does not follow [input](#input) format requirement, you need to modify the data to have correct format.
...
- `maxit` -  Integer specifying the maximum number of iterations for internal refinement process within a single `imputation` cycle. Default is 1.
- `outlier.rm` -  A logical value indicating whether outlier values should be removed during `imputation`. Default is TRUE.
