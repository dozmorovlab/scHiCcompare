...
You also can have option to save the results into chosen directory by parameter in `ScHiCcompare()` [function](#ScHiCcompare-function). This will save normalization result table, differential result table, and imputed cell scHi-C data (each group is a sub-folder). The sample of saved output folder structure is:
        ...
        + Imputed_{group 1's name}
          └── + imp_{cell name}.txt
        + Imputed_group 2's name 
          └── + imp_{cell name}.txt 

Normalization result `Bulk_normalization_table.txt` outputs 15 columns with contents:
...
Differential result table `Differential_analysis_table.txt` has 17 columns with first 15 columns same as normalization result table and additionally 2 columns of log fold change Z score [Z] and differential result cluster (1 - differential, 0 - non differential) [Difference.cluster].

Imputed cell's scHiC data is saved in folder for each group, which has modified output of five columns [chr1, start1, chr2, start2, IF].
