...
`ScHiCcompare()` [??? Rename the function to keep capitalization consistent with the package - scHiCcompare] function conducts a differential analysis workflow, including imputation and normalization, between chromosome-specific chromatin interaction matrices of two conditions (or two cell types groups). 

 - **Imputation** - First, scHiC data in each group can optionally undergo `imputation` to address data sparsity. As resolution increases, the percentage of '0' values or missing data also rises drastically. To address this, scHiCompare applies the random forest (`RF`) imputation method to pool bands - data subsets based on progressively increasing genomic distance ranges. There are two pooling strategies:

    1. `Progressive` pooling, in which each subsequent group combines more consecutive distances by gradually increasing the number of distances elements included by one.
    
    2. `Fibonacci` pooling, where each group includes an increasing number of consecutive distances, with the number of distances in subsequent groups following the Fibonacci sequence.
  
    Due to extreme sparsity at higher genomic distance and randomness of long-range interactions, scHiCcompare, by default, workflow focuses on distance 1 to 10MB (`main.Distance`).  If any pool band falls outside this range and has a percentage of missing values exceeding the specified threshold (default missPerc.threshold is 95%), the missing interaction frequencies (IFs) values are imputed using the mean IF values from the available data within that pool band.
 ...
 - **Normalization** - After obtaining two group-specific pseudo-bulk matrices, `normalization` removes global and local biases between two pseudo-bulk matrices. The `r BiocStyle::Biocpkg("ScHiCcompare")` workflow applies a loess [??? Search and replace to make capitalization consistent with the manuscript, Loess] regression model from `r BiocStyle::Biocpkg("HiCcompare")` to jointly normalize two imputed pseudo-bulk matrices. Briefly,
 ...
 - **Differential analysis** - `differential analysis` is performed on the processed pseudo-bulk matrices to identify differential chromatin interactions between the two cell types or conditions. This involves clustering the normalized log fold changes of interaction frequencies into difference and non-difference groups.
    + The non-difference group is assumed to follow a normal distribution centered around 0, identified using a Gaussian Mixture Model (GMM). The difference group consists of log fold changes belonging to other distributions clusters that deviate from the non-difference group's normal distribution. To improve precision, a control log fold change threshold `fprControl.logfc` is applied to difference cluster, excluding bins with low log fold change.
    + When the differences are not large enough to form distinct distributions, the differential analysis of `r BiocStyle::Biocpkg("HiCcompare")` is used.
...
[??? Where is chr2 in the displayed table? The data object does not represent 5 columns. 
Do we need this object in the first place, or can you demo the data format using some other example?
Can you use a file from "Prepare Input folders" section?]
[??? Data and object should be the same. If your data object is `ODC_chr22_1`,
it should be loaded as `data(ODC_chr22_1)`]
...
**Other sources**:
  ...
  To download human brain ODC and MG [??? clarify what are those abbreviations?] cell type (Lee et al., 2019), we used the `download_schic()` function of `r BiocStyle::Biocpkg("Bandnorm")` package to download the scHiC data of  ODC and MG cell types groups in 1MB resolution.
...
contact_matrix <- straw(
    ...
    chr1loc [??? The argument has the same name?] = "chr1",       # Chromosome 2 (same with chromosome 1for intra-chromosomal interactions)
...
# Create the 'ODCs_example' folder within the current directory. [??? That's not ODC]
...
for (dataset_file in dataset_files) {
  dataset_name <- tools::file_path_sans_ext(basename(dataset_file)) 
  load(dataset_file)
  # Save the dataset into the 'ODCs_example' folder [??? That's not ODC]
  write.table( get(dataset_name), file = file.path(save_dir, paste0(dataset_name, ".txt")), sep = "\t", row.names = FALSE, col.names = FALSE, quote = FALSE)
}
...
- `select.chromosome` -  Integer or character indicating the chromosome to be analyzed (e.g., 'chr1' or 'chrX'.) [??? Why Integer? don't we always use character here?]
- `main.Distances`- Numeric vector or character ('full' [??? Can you use Inf here? That's infinity, and handled as numeric. We typically don't mix types in one argument]) indicating the range (or full range) of interacting genomic distances (in base pairs) between two regions (e.g., loci or bins) to focus on. [??? "As such, the number of genomic distance bins will be automatically selected" - add if this is correct. And what if you specify weird range, like 1:2? or 1:Inf?] As the distance range and resolution increase, the percentage of '0' or missing values also increases. Selecting 'full' or a large distance range at high resolution (e.g., below 200kb) can make the function take longer to run due to extreme sparsity. By default, scHiCcompare detects differences within biologically relevant distance range from 1 to 10MB (`main.Distances` = 1:10000000).
...
ScHiCcompare(file.path.1, file.path.2,
             ...
             n.imputation = 5,  maxit = 1, outlier.rm = TRUE, missPerc.threshold = 95,
...
- `maxit` -  Integer specifying the maximum number of iterations for internal refinement [??? of what?] process within a single `imputation` cycle. Default is 1. [??? I don't see useful value for it.]
- `outlier.rm` -  A logical value indicating whether outlier values [??? defined as?] should be removed during `imputation`. Default is TRUE.
...
The `ScHiCcompare()` function will return an object that containes plots, differential results, pseudo-bulk matrices in sparse format, normalized results, and imputation tables. The full differential result are available in in `$Differential_Analysis`. Intermediate results can be accessed with `$Intermediate`, including imputation result table (`$Intermediate$Imputation`), speudo-bulk matrix in sparse format (`$Intermediate$PseudoBulk`), and normalization table (`$Intermediate$Bulk.Normalization`).

You also can have option to save the results into chosen directory by parameter in `ScHiCcompare()` [function](#ScHiCcompare-function). This will save normalization result table, differential result table, and imputed scHi-C in sparse format (each group is a sub-folder). The sample of saved output folder structure is:
 
        + Bulk_normalization_table.txt
        + Differential_analysis_table.txt
        + Imputed_{group 1's name}
          └── + imp_{cell name}.txt
        + Imputed_Condition2_cells [??? Doesn't match the naming in group 1]
          └── + imp_{group 2's name}.txt [??? Doesn't match the naming in group 1]

[??? Document the columns of these tables,
and the contend of the folders]
...
The output of `$Intermediate$Imputation` of `ScHiCcompare()` function has achieved this format of input table. For more details, see the sections on [Output](#output), [ScHiCompare workflow](#ScHiCompare-workflow) )
...
[??? Instead of IF1, IF2 etc columns, use the original file names.
Those file names correspond to individual cells. User wants to trace how imputed
values correspond to the original ones.]

```{r, echo=FALSE}
DT::datatable(imp_ODC_table, options = list(scrollX = TRUE), width = 700)
```

Therefore, we need to create the table input for original IFs values in the same format. Below is continuous example from [Example of real anlysis](#Example-of-real-anlysis) above, showing how you can construct this table for original IF values and compare them with the output fro of imputed IF values.
