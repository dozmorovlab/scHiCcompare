...
[??? If I execute previous code chunks, this function cannot be run.
Error in read_files(file.path = file.path, type = "txt", txt.sparse.heads.position = c(1,  : 
  Error: No .txt files found in the specified directory.]
...
Normalization result [??? Which slot from the results, `$Differential_Analysis`?] `Bulk_normalization_table.txt` outputs 15 columns with contents:
...
Differential result table `Differential_analysis_table.txt` has 17 columns with first 15 columns same as normalization result table and additionally 2 columns of log fold change Z score [Z] and differential result cluster (1 - differential, 0 - non differential) [Difference.cluster]. [??? OK, but why not including those columns in `$Differential_Analysis`? would be simpler to write]

Imputed cell's scHiC data is saved in folder for each group, which has modified [??? or is it sparse upper triangular?] output of five columns [chr1, start1, chr2, start2, IF].
