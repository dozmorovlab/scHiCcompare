...
# Preallocate a list to store data frames
data_list_ODC <- vector("list", length(file.names))
# Read all data files into the list
for(i in seq_along(file.names)) {
  data = read.delim(file.names[[i]])
  names(data) = c('chr', 'region1', 'region2', paste0('IF_', i))
  data_list_ODC[[i]] <- data
}
# Use Reduce to merge all data frames in the list
scHiC.table_ODC = Reduce(function(x, y) full_join(x, y, by = c("region1", "region2", "chr")), data_list_ODC, init = scHiC.table_ODC)
...
# Preallocate a list to store data frames
data_list_MG <- vector("list", length(file.names))
# Read all data files into the list
for(i in seq_along(file.names)) {
  data = read.delim(file.names[[i]])
  names(data) = c('chr', 'region1', 'region2', paste0('IF_', i))
...
# Use Reduce to merge all data frames in the list
scHiC.table_MG = Reduce(function(x, y) full_join(x, y, by = c("region1", "region2", "chr")), data_list_MG, init = scHiC.table_MG)
