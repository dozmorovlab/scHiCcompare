...
```{r, echo=FALSE}
data(mod.ODC_chr22_1) [??? The name of the data should be the same as the returned object]
names(ODC_chr22_1) <- c('chr' , 'start1', 'start2', 'IF') [??? Make the object with those column names at the first place]
...
## Download scHiC data
  ...
  You can also download these data by R. Example below show steps to download mices [??? so many typos, correct, i do not understand] scHi-C dataset (Flyamer et al.2017) on [GEO](#https://www.ncbi.nlm.nih.gov/geo/).
...
result = ScHiCcompare(file.path.1 = 'mod_ODCs_example', file.path.2 = 'mod_MGs_example', [??? Can't run]
