...
[??? We need simple .txt files, gzipped. People do not start with .rda files.]
...
source_folder <- system.file("ODCs_example", package = "scHiCcompare")
destination_folder <- file.path(getwd(), "ODCs_example")
...
# Loop over each file, unzip, and save
for (file in file_list) {
  # Define output file name (remove .gz extension)
  output_file <- file.path(destination_folder, sub("\\.gz$", "", basename(file)))
  # Read the gzipped file
  data <- read.table(gzfile(file), header = TRUE)
  # Write to a new .txt file without compression
  write.table(data, file = output_file, row.names = FALSE, col.names = TRUE, quote = FALSE)
}
